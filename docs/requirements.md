...
| SLP-LIFE-009 | When a deployment configures a fresh fast thermal-admission projection, warning, sleep, cutoff, recovery, stale, malformed, or unavailable state must reject every new inference acquisition before request-body processing or wake with HTTP 503, `Retry-After`, and `error.type=thermal_cooldown`. An already leased buffered or streaming request may drain; `POST /sleep` quiesces later acquisitions before sleeping. The deployment watchdog owns temperature thresholds and emergency cutoff ordering. | thermal admission, manager drain, and server response tests |
