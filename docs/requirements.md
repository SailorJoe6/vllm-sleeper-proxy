...
compatibility with bodyless loopback `POST /sleep`. This stage does not expose a
new action-scoped lifecycle endpoint. A future action-scoped thermal control
must remain local-only and may be enabled only when the root durable record and
caller atomically provide the matching validated action ID and phase.
